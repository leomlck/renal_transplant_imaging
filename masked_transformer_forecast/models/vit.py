...
from models.vit_utils import DropPath, trunc_normal_ #to_2tuple, trunc_normal_
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        B, C, T, H, W, D = x.shape
        x = rearrange(x, 'b c t h w d-> (b t) c h w d')
        x = self.proj(x)
        W = x.size(-1)
...
class TimeSformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, num_classes=400, num_frames=8, attention_type='divided_space_time',  pretrained_model='', **kwargs):
        ...
        self.model = VisionTransformer(img_size=img_size, in_chans=1, num_classes=num_classes, patch_size=patch_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=1, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1, num_frames=num_frames, attention_type=attention_type, **kwargs)
        ...
        if self.pretrained:
            load_pretrained(self.model, num_classes=self.model.num_classes, in_chans=1, filter_fn=_conv_filter, img_size=img_size, num_frames=num_frames, num_patches=self.num_patches, attention_type=self.attention_type, pretrained_model=pretrained_model)
    def forward(self, x):
        x = self.model(x)
        return x
